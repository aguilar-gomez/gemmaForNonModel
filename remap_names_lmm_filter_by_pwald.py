...
#first line header
next(infile)
outfile.write("\t".join(["chr","rs","ps","n_miss","allele1","allele0","af","beta","se","logl_H1","l_remle","l_mle","p_wald","p_lrt","p_score","original_rs","original_chr","original_pos"])+"\n")

for line in infile:
	column=line.split("\t")
	if np.log10(float(column[12]))<float(threshold): #filter by significance of pwald
		rs_format=column[1]
